...
func ResolveFrameworkPricingConfig(
	...
	if fileConfig != nil && fileConfig.Pricing != nil {
		...
		if fileConfig.Pricing.ModelParametersURL != nil {
			...
			if raw == "" {
				// Blank explicitly disables remote model-parameters sync.
		...
		if fileConfig.Pricing.MCPLibraryURL != nil {
			...
			if raw == "" {
				// Blank explicitly disables remote MCP catalog sync.
	...
	if dbConfig != nil {
		...
		if dbConfig.ModelParametersURL != nil {
		...
		if dbConfig.MCPLibraryURL != nil {
			trimmed := strings.TrimSpace(*dbConfig.MCPLibraryURL)
...
func (c *Config) GetVectorStoreConfigRedacted(ctx context.Context) (*vectorstore.Config, error) {
	...
	if vectorStoreConfig.Type == vectorstore.VectorStoreTypePgvector {
		var pgvectorConfig vectorstore.PgvectorConfig
		switch configured := vectorStoreConfig.Config.(type) {
		case vectorstore.PgvectorConfig:
			pgvectorConfig = configured
		case *vectorstore.PgvectorConfig:
			if configured == nil {
		...
		redactedPgvectorConfig := pgvectorConfig
		redactedPgvectorConfig.ConnectionString = *redactedPgvectorConfig.ConnectionString.FullyRedacted()
		redactedVectorStoreConfig := *vectorStoreConfig
		redactedVectorStoreConfig.Config = redactedPgvectorConfig
		return &redactedVectorStoreConfig, nil
	}
	return nil, nil
}
