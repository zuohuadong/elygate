...
func TestParseSessionTreeFiltersDoesNotUseSessionIDAlias(t *testing.T) {
	ctx := &fasthttp.RequestCtx{}
	ctx.QueryArgs().Set("session_id", "parent-request")
	ctx.QueryArgs().Set("harness_session_id", "child-1")
	...
	filters := &logstore.SearchFilters{}
	parseSessionTreeFilters(ctx, filters)
	if filters.HarnessSessionID != "child-1" {
		t.Fatalf("HarnessSessionID = %q, want child-1", filters.HarnessSessionID)
	}
	if filters.ParentSessionID != "parent-1" {
		t.Fatalf("ParentSessionID = %q, want parent-1", filters.ParentSessionID)
	}
	if filters.AgentName != "reviewer" {
		t.Fatalf("AgentName = %q, want reviewer", filters.AgentName)
	}
	if filters.TreeSessionID != "parent-1" {
		t.Fatalf("TreeSessionID = %q, want parent-1", filters.TreeSessionID)
	}
	if filters.IsSubagent == nil || !*filters.IsSubagent {
		t.Fatalf("IsSubagent = %#v, want true", filters.IsSubagent)
	}
	if filters.IsFork == nil || *filters.IsFork {
		t.Fatalf("IsFork = %#v, want false", filters.IsFork)
	}
	if got := parseParentRequestIDFilter(ctx); got != "parent-request" {
		t.Fatalf("session_id alias = %q, want parent-request", got)
	}
}
