...
// parseSessionTreeFilters reads harness session/parent/agent identity from query
// parameters. session_id remains an alias of parent_request_id.
func parseSessionTreeFilters(ctx *fasthttp.RequestCtx, filters *logstore.SearchFilters) {
	if filters == nil {
		return
	}
	if id := strings.TrimSpace(string(ctx.QueryArgs().Peek("harness_session_id"))); id != "" {
		filters.HarnessSessionID = id
	}
	if id := strings.TrimSpace(string(ctx.QueryArgs().Peek("parent_session_id"))); id != "" {
		filters.ParentSessionID = id
	}
	if name := strings.TrimSpace(string(ctx.QueryArgs().Peek("agent_name"))); name != "" {
		filters.AgentName = name
	}
	if id := strings.TrimSpace(string(ctx.QueryArgs().Peek("tree_session_id"))); id != "" {
		filters.TreeSessionID = id
	}
	if value, ok, err := parseBoolQueryArg(ctx, "is_subagent"); err == nil && ok {
		filters.IsSubagent = &value
	}
	if value, ok, err := parseBoolQueryArg(ctx, "is_fork"); err == nil && ok {
