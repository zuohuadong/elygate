...
async function main() {
    ...
    const dbUrl = envContent.match(/DATABASE_URL=(.+)/)?.[1] || process.env.DATABASE_URL;
    
    if (!newSecret || !newSalt) {
        console.error('Missing ENCRYPTION_SECRET or ENCRYPTION_SALT in .env');
        process.exit(1);
    }
