...
`audience` and `client_id` are required; `client_secret`, `authorization_server_url`, and `scopes` are optional. `client_id` and `client_secret` support `env.VAR_NAME` and `vault.path` references — plain values also work (encrypted at rest, redacted in API responses).
