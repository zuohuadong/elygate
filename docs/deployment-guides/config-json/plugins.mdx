...
<Note>
**The `plugins` array controls explicitly opt-in plugins**: `safety`, `semantic_cache`, `otel`, `maxim`, `datadog` (enterprise), and custom plugins.
...
`safety` is an opt-in built-in plugin for deterministic regular-expression checks at the inference HTTP boundary. Rules are evaluated in declaration order and can inspect `input`, `output`, or `both`.

- `block` returns a `403` OpenAI-style safety error and does not call the model for input matches.
- `respond` returns a synthetic safe response for Chat Completions, Completions, and Responses endpoints.
- Output rules require `client.disable_content_logging: true` when a logs store is enabled, so unchecked provider output is not retained before the HTTP response guard runs.
- Streaming is rejected with `400` when output rules are configured. This is deliberate: partial streamed output cannot be recalled after a later match.
  ...
  "plugins": [
    {
      ...
      "config": {
        ...
        "rules": [
          {
            "id": "prompt-injection",
            "pattern": "(?i)ignore previous instructions|reveal the system prompt",
            "apply_to": "input",
            "action": "block"
          },
          {
            "id": "aws-access-key-output",
...
This plugin is a local deterministic policy layer, not a semantic moderation service. Use specific patterns for organization identifiers, secrets, and known prompt-injection phrases; use Enterprise Guardrails or an external safety provider when classification or model-based detection is required.
