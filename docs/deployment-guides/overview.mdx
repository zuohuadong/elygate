...
For the current Elygate small-business product profile, start with [Single-node PostgreSQL](/deployment-guides/single-node-postgres). It is the recommended path for one server, one Elygate process, and one PostgreSQL instance. The Kubernetes, Helm, cloud, and multinode guides remain available for other Bifrost distributions but are not part of the current Elygate single-node acceptance scope.
...
|---|---|---|
| Docker or a VM | [Single-node PostgreSQL](/deployment-guides/single-node-postgres) | Run one Elygate process with PostgreSQL-backed storage |
...
| Docker or a VM (OSS SQLite) | [Docker setup](/quickstart/gateway/setting-up#docker) | Run one process with a mounted data directory |
