...
| `bifrost.plugins.otel.config.traces_enabled` | Export traces (set `false` for a metrics-only profile; `collector_url` then not required) | `true` |
| `bifrost.plugins.otel.config.collector_url` | OTLP collector endpoint (required when traces enabled) | `""` |
| `bifrost.plugins.otel.config.trace_type` | Trace type (`genai_extension`, `vercel`, or `open_inference`); required when traces enabled | `"genai_extension"` |
| `bifrost.plugins.otel.config.protocol` | Transport protocol (`grpc` or `http`), shared by traces and metrics; required when traces or metrics enabled | `"grpc"` |
