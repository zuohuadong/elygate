...
| `bifrost.mcp.toolSyncInterval` | Global tool sync interval as a Go duration string in whole minutes (for example `10m`). Use `0s` to use the runtime default of 10 minutes; periodic sync cannot be turned off. Legacy `bifrost.client.mcpToolSyncInterval: 0` means the same default. | `10m` |
