...
| `allowed_models` | An allowlist; `["*"]` means all models. An entry starting with `regex:` is an RE2 pattern matched against the whole model name, case-insensitively |
| `blacklisted_models` | A denylist, which wins over the allowlist. `regex:` entries work the same way |
