...
<Note>
  Setting `path` on a non-builtin plugin loads native code into the gateway process, so it
  requires an authenticated admin session even if [dashboard authentication](/quickstart/gateway/setting-up-auth)
  is otherwise disabled. A remote (`http`/`https`) `path` is only fetched if it resolves to a
  public address - loopback, private, and link-local destinations are refused.
</Note>
