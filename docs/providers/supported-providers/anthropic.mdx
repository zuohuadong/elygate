...
| `compact-2026-01-12` | ✅ | ✅ | ✅ | ✅ (routes the request to InvokeModel or InvokeModelWithResponseStream, since the Converse API does not support compaction) | ✅ (compaction edit) |
| `tool-search-tool-2025-10-19` | ✅ | ✅ | ✅ | ✅ (routes the request to InvokeModel or InvokeModelWithResponseStream, since the Converse API does not support tool search) | ✅ (tool_search tool / defer_loading) |
...
Agentic clients such as Codex send no cache markers, so nothing is cached and every
turn pays full price for a prompt that barely changed. Bifrost can add the marker for
them: set `prompt_cache.auto_inject` on the provider and the first cacheable block is
marked automatically.

Claude is the primary case, and everything on this page about `cache_control` applies
to injected markers too. See [Prompt caching](/features/prompt-caching) for the configuration,
the injection point syntax, and the per-request override.
