...
`key_ids` is scoped to a provider config; it is not a top-level Virtual Key field. Send `["*"]` to allow every key for that provider, or a list of key IDs to restrict selection. An empty or omitted `key_ids` denies all keys.
