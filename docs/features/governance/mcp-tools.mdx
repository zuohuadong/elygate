...
<Note>
To bundle tools from one or more MCP servers into a single addressable endpoint served at `/mcp/<slug>`, see [Virtual MCPs](/mcp/virtual-mcps). The per-key allow-list described here also applies to the tools a Virtual MCP exposes.
</Note>
