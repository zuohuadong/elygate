...
<Note>
MCP servers in `claude_desktop_config.json` are for the **Chat tab only**. For MCP in the Code tab, configure servers in `~/.claude.json` or your project's `.mcp.json` file. See [MCP Gateway](/mcp/overview#bifrost-as-an-mcp-gateway) for full setup details.
</Note>
...
- [MCP Gateway](/mcp/overview#bifrost-as-an-mcp-gateway) - Full MCP server setup and tool filtering