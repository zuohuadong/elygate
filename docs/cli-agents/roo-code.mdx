...
Roo Code supports MCP (Model Context Protocol). You can connect it to Bifrost's MCP server to access all tools configured in Bifrost. See [MCP Gateway](/mcp/overview#bifrost-as-an-mcp-gateway) for setup instructions.
