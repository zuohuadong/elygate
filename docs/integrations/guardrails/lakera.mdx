...
The earlier retained messages provide conversation context; Lakera evaluates the latest interaction rather than independently reclassifying every old turn. Set a rule's `send_all_conversation_turns` to `false`, then use `max_turns_to_send` to include the current input plus a chosen number of preceding turns.
