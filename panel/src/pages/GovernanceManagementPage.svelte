<script lang="ts">
	...
	function scopeLabel(value: unknown): string {
		const scope = String(value ?? 'global');
		const labels: Record<string, [string, string]> = {
			global: ['全局', 'Global'], provider: ['供应商', 'Provider'], provider_key: ['供应商密钥', 'Provider key'],
			virtual_key: ['虚拟密钥', 'Virtual key'], virtual_key_provider: ['虚拟密钥 + 供应商', 'Virtual key + provider'],
			virtual_key_provider_key: ['虚拟密钥 + 供应商密钥', 'Virtual key + provider key'], user: ['用户', 'User'],
			user_provider: ['用户 + 供应商', 'User + provider'], user_provider_key: ['用户 + 供应商密钥', 'User + provider key'],
		};
		const label = labels[scope];
		return label ? text(label[0], label[1]) : scope;
	}
	function matchTypeLabel(value: unknown): string {
		return value === 'wildcard' ? text('通配匹配', 'Wildcard') : value === 'exact' ? text('精确匹配', 'Exact') : displayValue(value);
	}
	function requestTypeLabel(value: string): string {
		const labels: Record<string, [string, string]> = {
			chat_completion: ['聊天补全', 'Chat completion'], text_completion: ['文本补全', 'Text completion'], responses: ['Responses 接口', 'Responses'],
			embedding: ['向量嵌入', 'Embedding'], rerank: ['重排序', 'Rerank'], speech: ['语音合成', 'Speech'], transcription: ['音频转写', 'Transcription'],
			image_generation: ['图像生成', 'Image generation'], image_variation: ['图像变体', 'Image variation'], image_edit: ['图像编辑', 'Image edit'],
			video_generation: ['视频生成', 'Video generation'], video_remix: ['视频重混', 'Video remix'], ocr: ['OCR 识别', 'OCR'],
		};
		const label = labels[value];
		return label ? text(label[0], label[1]) : value;
	}
	function requestTypesLabel(value: unknown): string {
		return Array.isArray(value) ? value.map(String).map(requestTypeLabel).join('、') : displayValue(value);
	}
	...
	function title(): string {
		...
		if (resourceName === 'provider-governance') return text('供应商治理', 'Provider governance');
		return text('价格覆盖', 'Pricing overrides');
	}
	function description(): string {
		if (resourceName === 'teams') return text('管理团队归属、多周期预算、令牌/请求限流和日历对齐重置。', 'Manage customer assignment, multi-window budgets, token/request limits, and calendar-aligned resets.');
		if (resourceName === 'customers') return text('管理客户级预算和限流，作为其团队与虚拟密钥的上层治理边界。', 'Manage customer-level budgets and limits as the parent governance boundary for teams and virtual keys.');
		if (resourceName === 'provider-governance') return text('为每个供应商设置全模型预算和限流；未配置的供应商也可直接启用治理。', 'Set all-model budgets and limits per provider; governance can be enabled for providers without existing settings.');
		return text('按全局、供应商、供应商密钥、虚拟密钥或用户范围覆盖模型定价，并精确限定请求类型和模型模式。', 'Override model pricing by global, provider, key, virtual-key, or user scope with request-type and model-pattern matching.');
	}
	function validationMessage(cause: unknown): string {
		...
		const messages: Record<string, string> = {
			...
			'provider-required': text('请选择供应商。', 'Provider is required.'),
			'user-required': text('该范围必须填写用户 ID。', 'This scope requires a user ID.'),
			'virtual-key-required': text('该范围必须填写虚拟密钥 ID。', 'This scope requires a virtual-key ID.'),
			'provider-key-required': text('该范围必须填写供应商密钥 ID。', 'This scope requires a provider-key ID.'),
			'budget-duration-duplicate': text('每个预算必须使用不同的重置周期。', 'Each budget must use a distinct reset window.'),
			'token-limit-invalid': text('令牌上限必须是至少 1 的整数。', 'Token limit must be an integer of at least 1.'),
	...
	async function remove(record: JsonRecord): Promise<void> {
		...
		try {
			...
			notice = entityKind === 'provider' ? text('供应商治理已重置。', 'Provider governance reset.') : text('记录已删除。', 'Record deleted.'); await load();
