<script lang="ts">
	...
	function providerStatusLabel(status: string): string {
		if (status === 'active') return i18n.locale === 'zh-CN' ? '运行中' : 'Active';
		if (status === 'inactive' || status === 'disabled') return i18n.t('elygate.disabled');
		if (status === 'error' || status === 'unhealthy') return i18n.locale === 'zh-CN' ? '异常' : 'Error';
		return status || '—';
	}

	function providerKeyStatusLabel(key: JsonRecord): string {
		if (key.enabled === false) return i18n.t('elygate.disabled');
		const status = stringValue(key, 'status');
		if (status === 'success' || status === 'active') return i18n.locale === 'zh-CN' ? '验证成功' : 'Verified';
		if (status === 'error' || status === 'failed') return i18n.locale === 'zh-CN' ? '验证失败' : 'Failed';
		return status || i18n.t('elygate.enabled');
	}
	...
	let deletingProviderName = $state('');
	let error = $state('');
	let notice = $state('');
	let warning = $state('');
	...
	function hasActiveMutation(): boolean {
		return isSaving || revalidatingKeyId !== '' || deletingKeyId !== '' || deletingProviderName !== '';
	}
	...
	async function loadProviders(): Promise<void> {
		...
		error = '';
		try {
			const nextProviders = getListPayload(await requestJson('/api/providers'));
			if (generation === providerLoadGeneration) providers = nextProviders;
		} catch (cause) {
			if (generation === providerLoadGeneration) error = displayError(cause, i18n.t('elygate.loadFailed'));
		} finally {
			if (generation === providerLoadGeneration) isLoading = false;
		}
	}
	...
	async function removeProvider(provider: JsonRecord): Promise<void> {
		...
		if (hasActiveMutation() || !name || !window.confirm(i18n.t('elygate.confirmDelete'))) return;
	...
	async function saveKey(): Promise<void> {
		...
		if (hasActiveMutation()) return;
		const providerContext = selectedProvider;
		isSaving = true;
		error = '';
		try {
			...
			if (editingKey) {
				await requestJson(`/api/providers/${encodePathSegment(providerContext)}/keys/${encodePathSegment(stringValue(editingKey, 'id'))}`, {
					method: 'PUT',
					body: JSON.stringify(payload),
				});
			} else {
				await requestJson(`/api/providers/${encodePathSegment(providerContext)}/keys`, { method: 'POST', body: JSON.stringify(payload) });
			}
			if (selectedProvider === providerContext) {
				editingKey = null;
				keyForm = emptyKeyForm();
				notice = i18n.t('elygate.save');
				await loadKeys(providerContext);
			}
		} catch (cause) {
			error = displayError(cause, i18n.t('elygate.operationFailed'));
		} finally {
	...
	async function removeKey(key: JsonRecord): Promise<void> {
		...
		if (hasActiveMutation() || !selectedProvider || !id || !window.confirm(i18n.t('elygate.confirmDelete'))) return;
		const providerContext = selectedProvider;
		deletingKeyId = id;
		error = '';
		try {
			await requestJson(`/api/providers/${encodePathSegment(providerContext)}/keys/${encodePathSegment(id)}`, { method: 'DELETE' });
			clearDeletedKeyFromEditor(id);
			notice = i18n.t('elygate.delete');
			if (selectedProvider === providerContext) await loadKeys(providerContext);
		} catch (cause) {
	...
	async function revalidateKey(key: JsonRecord): Promise<void> {
		const id = stringValue(key, 'id');
		if (hasActiveMutation() || !selectedProvider || !id) return;
		const providerContext = selectedProvider;
		...
		try {
			...
			await requestJson(`/api/providers/${encodePathSegment(providerContext)}/keys/${encodePathSegment(id)}`, {
				method: 'PUT',
				body: JSON.stringify(payload),
			});
			if (selectedProvider === providerContext) {
...
{#snippet keyWorkspace()}
	<div class="provider-key-workspace">
		{#if warning}<div class="notice warning" role="status">{warning}</div>{/if}
			<div class="key-layout"><div class="table-wrap" aria-busy={isKeysLoading}><table><thead><tr><th>{i18n.t('elygate.keyName')}</th><th>{i18n.t('elygate.status')}</th><th>{i18n.t('elygate.models')}</th><th>{i18n.t('elygate.actions')}</th></tr></thead><tbody>{#each keys as key (stringValue(key, 'id'))}<tr><td>{stringValue(key, 'name')}</td><td title={stringValue(key, 'description')}>{providerKeyStatusLabel(key)}</td><td>{keyModelsLabel(key)}</td><td class="actions"><button type="button" disabled={hasActiveMutation()} onclick={() => editKey(key)}>{i18n.t('elygate.edit')}</button><button type="button" onclick={() => void revalidateKey(key)} disabled={hasActiveMutation()}>{i18n.t('elygate.revalidate')}</button><button class="danger" type="button" onclick={() => void removeKey(key)} disabled={hasActiveMutation()}>{i18n.t('elygate.delete')}</button></td></tr>{:else}<tr><td colspan="4" class="empty">{isKeysLoading ? i18n.t('elygate.loading') : i18n.t('elygate.noResults')}</td></tr>{/each}</tbody></table></div>
			<form class="key-form" autocomplete="off" onsubmit={submitKey}><h3>{editingKey ? i18n.t('elygate.edit') : i18n.t('elygate.create')} {i18n.t('elygate.keyName')}</h3><label>{i18n.t('elygate.keyName')}<input bind:value={keyForm.name} autocomplete="off" required disabled={hasActiveMutation()} /></label><label>{i18n.t('elygate.keyValue')}<input type="password" bind:value={keyForm.value} autocomplete="new-password" disabled={hasActiveMutation()} /><small>{i18n.t('elygate.keyValueHelp')}</small></label><label>{i18n.t('elygate.modelsCsv')}<input bind:value={keyForm.models} placeholder="*" disabled={hasActiveMutation()} /><small>{i18n.t('elygate.keyModelsHelp')}</small></label><label>{i18n.t('elygate.blacklistedModelsCsv')}<input bind:value={keyForm.blacklistedModels} disabled={hasActiveMutation()} /></label><div class="grid-two"><label>{i18n.t('elygate.weight')}<input type="number" min="0" step="0.01" bind:value={keyForm.weight} disabled={hasActiveMutation()} /></label><label class="check"><input type="checkbox" bind:checked={keyForm.enabled} disabled={hasActiveMutation()} /> {i18n.t('elygate.enabled')}</label></div><label>{i18n.t('elygate.advancedJson')}<textarea bind:value={keyForm.advanced} rows="5" disabled={hasActiveMutation()}></textarea></label><footer><button type="button" disabled={hasActiveMutation()} onclick={() => { editingKey = null; keyForm = emptyKeyForm(); }}>{i18n.t('elygate.cancel')}</button><button class="primary" type="submit" disabled={hasActiveMutation()}>{i18n.t('elygate.save')}</button></footer></form>
		</div>
	</div>
{/snippet}
...
<section class="page-shell" data-resource={resourceName}>
	<header class="page-heading">
		<div><p class="eyebrow">{getAppName()} / {i18n.t('elygate.models')}</p><h1>{providerKeysPage ? i18n.t('elygate.providerKeys') : i18n.t('elygate.providers')}</h1><p>{providerKeysPage ? i18n.t('elygate.apiKeySeparateHint') : i18n.t('elygate.providerNameHelp')}</p></div>
		<div class="heading-actions"><button class="primary" type="button" onclick={() => void (providerKeysPage ? loadKeys() : loadProviders())} disabled={isLoading || isKeysLoading || hasActiveMutation()}>{i18n.t('elygate.refresh')}</button>{#if !providerKeysPage}<button class="primary" type="button" onclick={openCreate} disabled={hasActiveMutation()}>{i18n.t('elygate.create')}</button>{/if}</div>
	</header>
	{#if error}<div class="notice error" role="alert">{error}</div>{/if}
	{#if notice}<div class="notice success" role="status">{notice}</div>{/if}
	{#if providerKeysPage}
		<label class="provider-selector">{i18n.t('elygate.providers')}<select bind:value={selectedProvider} onchange={() => void selectProvider(selectedProvider)} disabled={isLoading || providers.length === 0 || hasActiveMutation()}>{#each providers as provider (stringValue(provider, 'name'))}<option value={stringValue(provider, 'name')}>{stringValue(provider, 'name')}</option>{/each}</select></label>
		{@render keyWorkspace()}
	{:else}
		<div class="table-wrap" aria-busy={isLoading}>
			...
			{#each providers as provider (stringValue(provider, 'name'))}
					<tr><td><strong>{stringValue(provider, 'name')}</strong></td><td title={stringValue(provider, 'description')}>{providerStatusLabel(stringValue(provider, 'provider_status'))}</td><td>{stringValue((provider.network_config as JsonRecord | undefined) ?? {}, 'base_url') || '—'}</td><td class="actions"><button type="button" disabled={hasActiveMutation()} onclick={() => openEdit(provider)}>{i18n.t('elygate.edit')}</button><button type="button" disabled={hasActiveMutation()} onclick={() => void openKeys(provider)}>{i18n.t('elygate.manageKeys')}</button><button class="danger" type="button" disabled={hasActiveMutation()} onclick={() => void removeProvider(provider)}>{i18n.t('elygate.delete')}</button></td></tr>
...
{#if modal === 'create' || modal === 'edit'}
	<div class="modal-backdrop"><div class="modal" role="dialog" aria-modal="true" aria-labelledby="provider-dialog-title"><header><h2 id="provider-dialog-title">{modal === 'create' ? i18n.t('elygate.create') : i18n.t('elygate.edit')} {i18n.t('elygate.providers')}</h2><button type="button" onclick={() => (modal = null)}>{i18n.t('elygate.close')}</button></header>
		<form onsubmit={submitProvider}><div class="form-hint" role="note">{i18n.t('elygate.apiKeySeparateHint')}</div><label>{i18n.t('elygate.providerName')}<input bind:value={providerForm.name} required disabled={modal === 'edit' || hasActiveMutation()} /><small>{i18n.t('elygate.providerNameHelp')}</small></label>
			...
			<footer><button type="button" disabled={hasActiveMutation()} onclick={() => { if (!hasActiveMutation()) modal = null; }}>{i18n.t('elygate.cancel')}</button><button class="primary" type="submit" disabled={hasActiveMutation()}>{i18n.t('elygate.save')}</button></footer>
		</form>
	</div></div>
{/if}
