...
<section class="page-shell" data-resource={resourceName}>
	<header class="page-heading"><div><p class="eyebrow">Elygate / Governance</p><h1>{i18n.t('elygate.modelLimits')}</h1><p>{i18n.t('elygate.modelLimitsHint')}</p></div><button class="primary" type="button" onclick={openCreate}>+ {i18n.t('elygate.addLimit')}</button></header>
