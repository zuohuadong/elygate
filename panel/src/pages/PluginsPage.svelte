...
<section class="page-shell" data-resource={resourceName}>
	<header class="page-heading">
		<div><p class="eyebrow">{getAppName()} / Integrations</p><h1>{featureScoped ? text('插件能力配置', 'Plugin capability') : text('插件管理', 'Plugin management')}</h1><p>{featureScoped ? text('此页面由当前运行插件声明的能力自动提供；配置保存后插件会立即重新加载。', 'This page is provided by a capability declared by the active plugin; saving configuration reloads it immediately.') : text('安装和配置内置、自定义及企业插件，检查运行状态、钩子类型和加载日志，并控制自定义插件相对内置插件的执行顺序。', 'Install and configure built-in, custom, and enterprise plugins, inspect runtime status, hook types, and load logs, and control custom plugin execution around built-ins.')}</p></div>
