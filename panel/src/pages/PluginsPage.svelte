...
<section class="page-shell" data-resource={resourceName}>
	<header class="page-heading">
		<div><p class="eyebrow">Elygate / Integrations</p><h1>{featureScoped ? text('插件能力配置', 'Plugin capability') : text('插件管理', 'Plugin management')}</h1><p>{featureScoped ? text('此页面由当前运行插件声明的能力自动提供；配置保存后插件会立即重新加载。', 'This page is provided by a capability declared by the active plugin; saving configuration reloads it immediately.') : text('安装和配置内置、自定义及企业插件，检查运行状态、钩子类型和加载日志，并控制自定义插件相对内置插件的执行顺序。', 'Install and configure built-in, custom, and enterprise plugins, inspect runtime status, hook types, and load logs, and control custom plugin execution around built-ins.')}</p></div>
		...
		<div class:sequence-modal={modal === 'sequence'} class="modal" role="dialog" aria-modal="true" aria-labelledby="plugin-modal-title">
			...
			{#if modal === 'editor'}
				<div class="form-grid">
					...
					{#if draft.kind === 'custom'}<label class="span-2">{text('插件路径或 URL', 'Plugin path or URL')}<input bind:value={draft.path} placeholder="/opt/elygate/plugins/governance.so" /><small>{text('这是会在 Elygate 进程内加载的原生代码；后端要求真实管理员认证。', 'This native code loads inside the Elygate process; the backend requires genuine admin authentication.')}</small></label>{/if}
			...
			{#if modal === 'sequence'}
				...
				<div class="sequence-note">{text('提示：config.json 中显式声明的插件顺序可能在 Elygate 重启后覆盖数据库顺序。', 'Note: an explicit plugin sequence in config.json may override database order after Elygate restarts.')}</div>
