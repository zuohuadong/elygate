<script lang="ts">
	...
	function validationMessage(cause: unknown): string {
		...
		if (cause.message === 'boundary-order') return text('阈值必须满足简单 < 中等 < 复杂 < 推理。', 'Thresholds must increase from Simple to Medium to Complex to Reasoning.');
...
<section class="page-shell" data-resource={resourceName}>
	...
	{#if isComplexity}
			<section class="spectrum-card"><div class="section-heading"><div><h2>{text('复杂度光谱', 'Complexity spectrum')}</h2><p>{text('阈值变化会实时反映四个分类区间。', 'Threshold changes update all four classification ranges in real time.')}</p></div><div class="legend"><span>{text('简单', 'Simple')}</span><span>{text('中等', 'Medium')}</span><span>{text('复杂', 'Complex')}</span><span>{text('推理', 'Reasoning')}</span></div></div><div class="spectrum"><div class="simple" style:width={`${spectrum.simple}%`}>{text('简单', 'Simple')}</div><div class="medium" style:width={`${spectrum.medium}%`}>{text('中等', 'Medium')}</div><div class="complex" style:width={`${spectrum.complex}%`}>{text('复杂', 'Complex')}</div><div class="reasoning" style:width={`${spectrum.reasoning}%`}>{text('推理', 'Reasoning')}</div></div><div class="axis"><span>0</span><span>{Number(complexityDraft.simpleMedium).toFixed(2)}</span><span>{Number(complexityDraft.mediumComplex).toFixed(2)}</span><span>{Number(complexityDraft.complexReasoning).toFixed(2)}</span><span>1</span></div></section>
