<script lang="ts">
	...
	function validationMessage(cause: unknown): string {
		...
		if (cause.message === 'boundary-order') return text('阈值必须满足 Simple < Medium < Complex < Reasoning。', 'Thresholds must increase from Simple to Medium to Complex to Reasoning.');
...
<section class="page-shell" data-resource={resourceName}>
	<header class="page-heading"><div><p class="eyebrow">Elygate / {isComplexity ? 'Governance' : 'System'}</p><h1>{isComplexity ? text('复杂度路由', 'Complexity router') : text('全局代理', 'Global proxy')}</h1><p>{isComplexity ? text('配置四级复杂度阈值和关键词库，生成可供路由规则匹配的 complexity_tier。', 'Tune four complexity tiers and keyword dictionaries that produce complexity_tier for routing rules.') : text('控制 SCIM、推理和外部 API 请求的全局 HTTP(S) 代理、认证、绕过列表和 TLS 行为。', 'Control the global HTTP(S) proxy, authentication, bypass list, and TLS behavior for SCIM, inference, and external API requests.')}</p></div><div class="heading-actions"><button type="button" onclick={() => void load()} disabled={isLoading}>{text('刷新', 'Refresh')}</button>{#if isComplexity}<button type="button" onclick={() => void resetComplexity()} disabled={isSaving}>{text('恢复默认', 'Restore defaults')}</button>{/if}<button class="primary" type="button" onclick={() => void save()} disabled={isSaving || isLoading}>{isSaving ? text('保存中…', 'Saving…') : text('保存', 'Save')}</button></div></header>
	{#if error}<div class="notice error" role="alert">{error}</div>{/if}
	{#if notice}<div class="notice success" role="status">{notice}</div>{/if}
	{#if isComplexity}
		<section class="spectrum-card"><div class="section-heading"><div><h2>{text('复杂度光谱', 'Complexity spectrum')}</h2><p>{text('阈值变化会实时反映四个分类区间。', 'Threshold changes update all four classification ranges in real time.')}</p></div><div class="legend"><span>Simple</span><span>Medium</span><span>Complex</span><span>Reasoning</span></div></div><div class="spectrum"><div class="simple" style:width={`${spectrum.simple}%`}>SIMPLE</div><div class="medium" style:width={`${spectrum.medium}%`}>MEDIUM</div><div class="complex" style:width={`${spectrum.complex}%`}>COMPLEX</div><div class="reasoning" style:width={`${spectrum.reasoning}%`}>REASONING</div></div><div class="axis"><span>0</span><span>{Number(complexityDraft.simpleMedium).toFixed(2)}</span><span>{Number(complexityDraft.mediumComplex).toFixed(2)}</span><span>{Number(complexityDraft.complexReasoning).toFixed(2)}</span><span>1</span></div></section>
		<section class="settings-card"><div class="section-heading"><div><h2>{text('分级阈值', 'Tier boundaries')}</h2><p>{text('必须严格递增，且每个值位于 0 与 1 之间。', 'Values must increase strictly and remain between 0 and 1.')}</p></div></div><div class="boundary-grid"><label><span>Simple → Medium</span><input type="number" min="0.01" max="0.99" step="0.01" bind:value={complexityDraft.simpleMedium} /></label><label><span>Medium → Complex</span><input type="number" min="0.01" max="0.99" step="0.01" bind:value={complexityDraft.mediumComplex} /></label><label><span>Complex → Reasoning</span><input type="number" min="0.01" max="0.99" step="0.01" bind:value={complexityDraft.complexReasoning} /></label></div></section>
		<section class="keywords"><div class="section-heading"><div><h2>{text('关键词库', 'Keyword dictionaries')}</h2><p>{text('每行或逗号分隔一个短语；保存时自动去重和清理空白。', 'Enter one phrase per line or comma-separated; saving de-duplicates and trims values.')}</p></div></div><div class="keyword-grid">{#each COMPLEXITY_KEYWORD_KEYS as key (key)}<article><div><h3>{text(keywordMeta[key].zh, keywordMeta[key].en)}</h3><span>{keywordCount(complexityDraft.keywords[key])}</span></div><p>{text(keywordMeta[key].helpZh, keywordMeta[key].helpEn)}</p><textarea rows="12" bind:value={complexityDraft.keywords[key]}></textarea></article>{/each}</div></section>
	{:else}
