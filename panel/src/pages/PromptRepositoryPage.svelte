<script lang="ts">
	...
	async function selectPrompt(prompt: JsonRecord): Promise<void> {
		isLoading = true;
		error = '';
		try {
...
<style>
	...
	.workspace { display: grid; gap: .7rem; grid-template-columns: 210px 260px minmax(0, 1fr); }
	...
	.folders > div > button:first-child { align-items: center; display: flex; justify-content: space-between; text-align: left; }
	.prompts span, .row span { color: var(--muted-foreground); display: block; font-size: .72rem; margin-top: .2rem; }
	...
	@media (max-width: 980px) { .workspace { grid-template-columns: 190px 1fr; } .detail { grid-column: 1 / -1; } .folders, .prompts { max-height: 280px; } }
