...
<section class="page-shell" data-resource={resourceName}>
	<header class="page-heading"><div><p class="eyebrow">Elygate / Integrations</p><h1>Webhook</h1><p>{text('异步任务完成后会通知所有订阅对应事件的启用端点；可通过 x-bf-async-webhook 仅指定一个端点。', 'Async jobs notify every enabled endpoint subscribed to the terminal event; use x-bf-async-webhook to target only one endpoint.')}</p></div><button class="primary" type="button" onclick={openCreate}>+ {text('添加端点', 'Add endpoint')}</button></header>
