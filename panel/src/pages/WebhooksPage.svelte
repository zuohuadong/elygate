...
<section class="page-shell" data-resource={resourceName}>
	<header class="page-heading"><div><p class="eyebrow">{getAppName()} / Integrations</p><h1>Webhook</h1><p>{text('异步任务完成后会通知所有订阅对应事件的启用端点；可通过 x-bf-async-webhook 仅指定一个端点。', 'Async jobs notify every enabled endpoint subscribed to the terminal event; use x-bf-async-webhook to target only one endpoint.')}</p></div><button class="primary" type="button" onclick={openCreate}>+ {text('添加端点', 'Add endpoint')}</button></header>
...
<style>
	...
	.actions, .badges, .pagination div, .detail-actions { display: flex; flex-wrap: wrap; gap: .5rem; }
	.toolbar { align-items: end; display: grid; gap: .65rem; grid-template-columns: minmax(260px, 1.7fr) minmax(190px, .8fr) minmax(130px, .55fr) auto auto; margin: 1rem 0 .8rem; }
	...
	.toolbar select[multiple] { min-height: 4.6rem; }
	...
	table { border-collapse: collapse; min-width: 980px; table-layout: fixed; width: 100%; }
	.deliveries table { min-width: 1120px; }
	th, td { border-bottom: 1px solid var(--border); font-size: .78rem; padding: .7rem .65rem; text-align: left; vertical-align: middle; }
	.endpoints-table th:nth-child(1) { width: 12%; } .endpoints-table th:nth-child(2) { width: 22%; } .endpoints-table th:nth-child(3) { width: 16%; } .endpoints-table th:nth-child(4) { width: 8%; } .endpoints-table th:nth-child(5) { width: 8%; } .endpoints-table th:nth-child(6) { width: 14%; } .endpoints-table th:nth-child(7) { width: 20%; }
	.deliveries th:nth-child(1) { width: 13%; } .deliveries th:nth-child(2) { width: 12%; } .deliveries th:nth-child(3) { width: 18%; } .deliveries th:nth-child(4) { width: 8%; } .deliveries th:nth-child(5) { width: 13%; } .deliveries th:nth-child(6) { width: 8%; } .deliveries th:nth-child(7) { width: 16%; } .deliveries th:nth-child(8) { width: 12%; }
	...
	.status-button { text-decoration: none; }
	td .actions { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); }
	td .actions button { min-width: 0; white-space: nowrap; }
	...
	@media (max-width: 760px) { .page-shell { padding: 1rem; } .page-heading, .pagination, .detail-actions { align-items: stretch; flex-direction: column; } .toolbar, .form-grid, .detail-grid { grid-template-columns: 1fr; } .span-2 { grid-column: auto; } dl div { grid-template-columns: 1fr; } }
</style>
