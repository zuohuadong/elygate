<script lang="ts">
	...
	async function toggleFeature(flag: JsonRecord): Promise<void> {
		...
		try {
			...
			notice = flag.enabled === true ? text('功能开关已停用。', 'Feature flag disabled.') : text('功能开关已启用。', 'Feature flag enabled.');
			await load();
		} catch (cause) { error = displayError(cause, text('功能开关更新失败。', 'Failed to update feature flag.')); }
		finally { busyId = ''; }
	}
...
<section class="page-shell" data-resource={resourceName}>
	<header class="page-heading"><div><p class="eyebrow">{getAppName()} / {i18n.t(isFeatureFlags ? 'elygate.system' : 'elygate.observability')}</p><h1>{isFeatureFlags ? text('功能开关', 'Feature flags') : text('UA 映射', 'UA mappings')}</h1><p>{isFeatureFlags ? text('查看代码注册、数据库、远程配置与部署配置的最终来源；锁定或未注册项不会被误切换。', 'Inspect effective values from code defaults, database, remote config, and deployment configuration; locked or unregistered flags cannot be toggled.') : text('将请求 User-Agent 识别为日志中的应用名称和安全的栅格 Logo，支持包含、前缀、精确和正则匹配。', 'Map request User-Agent values to app names and safe raster logos in logs using contains, prefix, exact, or regex matching.')}</p></div>{#if !isFeatureFlags}<button class="primary" type="button" onclick={openCreate}>+ {text('添加映射', 'Add mapping')}</button>{/if}</header>
	...
	{#if isFeatureFlags}
		<div class="flag-list" class:loading={isLoading}>{#each filtered as flag (idOf(flag))}<article><div class="flag-main"><div class="flag-title"><strong>{titleOf(flag)}</strong>{#if flag.display_name}<code>{idOf(flag)}</code>{/if}<span class="badge">{columnValueFor(i18n.locale === 'zh-CN' ? 'zh-CN' : 'en', 'source', flag.source ?? 'default')}</span>{#if flag.enterprise_only === true}<span class="badge enterprise">{text('企业功能', 'Enterprise')}</span>{/if}{#if flag.locked === true}<span class="badge locked">🔒 {text('已锁定', 'Locked')}</span>{/if}{#if flag.registered === false}<span class="badge stale">{text('未注册', 'Unregistered')}</span>{/if}</div><p>{String(flag.description ?? '')}</p>{#if flag.registered === false}<small>{text('该值已存储，但当前版本没有注册此开关；请恢复注册或清理外部覆盖。', 'This value is stored but no code currently registers it; restore registration or remove the external override.')}</small>{/if}{#if flag.locked === true}<small>{text('该值由部署配置固定，请在部署环境中修改。', 'The value is pinned by deployment configuration; edit the deployment environment.')}</small>{/if}</div><button class:enabled={flag.enabled === true} class="toggle" type="button" disabled={busyId === idOf(flag) || flag.locked === true || flag.registered === false} onclick={() => void toggleFeature(flag)}>{flag.enabled === true ? text('已启用', 'Enabled') : text('已停用', 'Disabled')}</button></article>{:else}<div class="empty">{isLoading ? text('加载中…', 'Loading…') : text('暂无功能开关。', 'No feature flags.')}</div>{/each}</div>
	{:else}
