...
<section class="page-shell" data-resource={resourceName}>
	...
	<form class="toolbar" onsubmit={(event) => { event.preventDefault(); void refreshData(true); }}>
		...
		{#if mode === 'sessions'}<label>{text('精确身份', 'Exact identity')}<input bind:value={identity} placeholder={text('用户 / 虚拟密钥 / 会话 ID', 'user / vk / session id')} /></label>{/if}
		...
		{/if}
		{#if mode === 'sessions'}
			<label>{text('凭据类型', 'Kind')}<select multiple bind:value={kinds}><option value="token">{i18n.t('elygate.mcp.kind.token')}</option><option value="flow">{i18n.t('elygate.mcp.kind.flow')}</option><option value="header">{i18n.t('elygate.mcp.kind.header')}</option></select></label>
			<label>{text('状态', 'Status')}<select multiple bind:value={statuses}><option value="active">{i18n.t('elygate.mcp.status.active')}</option><option value="pending">{i18n.t('elygate.mcp.status.pending')}</option><option value="orphaned">{i18n.t('elygate.mcp.status.orphaned')}</option><option value="needs_reauth">{i18n.t('elygate.mcp.status.needs_reauth')}</option><option value="needs_update">{i18n.t('elygate.mcp.status.needs_update')}</option></select></label>
		{/if}
		{#if mode === 'sessions' || mode === 'grants'}
			<label>{text('绑定方式', 'Binding mode')}<select multiple bind:value={authModes}><option value="user">{i18n.t('elygate.mcp.binding.user')}</option><option value="vk">{i18n.t('elygate.mcp.binding.vk')}</option><option value="session">{i18n.t('elygate.mcp.binding.session')}</option></select></label>
		{/if}
		...
		<div class="table-wrap" class:loading={isLoading}><table><thead><tr>
			...
			{#each records as record (recordId(record))}
				<tr>
					...
					{#if mode === 'sessions'}<td><strong>{displayValue(isJsonRecord(record.mcp_client) ? record.mcp_client.name || record.mcp_client.client_id : '—')}</strong></td><td>{record.kind === 'header' || record.auth_kind === 'headers' ? i18n.t('elygate.mcp.kind.header') : 'OAuth'} · {record.kind === 'flow' ? i18n.t('elygate.mcp.kind.flow') : record.kind === 'token' ? i18n.t('elygate.mcp.kind.token') : displayValue(record.kind)}</td><td>{record.auth_mode === 'user' ? displayValue(isJsonRecord(record.user) ? record.user.name || record.user.email || record.user_id : record.user_id) : record.auth_mode === 'vk' ? displayValue(isJsonRecord(record.virtual_key) ? record.virtual_key.name || record.virtual_key.id : '—') : displayValue(record.session_id)}</td><td><span class="badge">{i18n.t(`elygate.mcp.status.${record.status}` as 'elygate.mcp.status.active')}</span></td><td>{displayDate(record.expires_at)}</td><td>{displayDate(record.created_at)}</td><td><div class="actions"><button type="button" onclick={() => openDetail(record)}>{text('详情', 'Details')}</button>{#if record.kind === 'flow' || (record.status !== 'orphaned' && record.can_reauth === true)}<button class="primary" type="button" onclick={() => void sessionAction(record, 'reauth')}>{record.kind === 'header' ? text('编辑值', 'Edit values') : record.kind === 'flow' ? text('完成认证', 'Complete auth') : text('重新认证', 'Reauthenticate')}</button>{/if}<button class="danger" type="button" onclick={() => void sessionAction(record, 'revoke')}>{text('撤销', 'Revoke')}</button></div></td>{/if}
