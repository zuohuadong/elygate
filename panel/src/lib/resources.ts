...
import type { EnterpriseMenuGroup, EnterpriseResourceManifestEntry } from './enterprise-panel';
...
const RESOURCE_SPECS: ResourceSpec[] = [
	...
	['request-logs', 'elygate.logs', 'scroll-text'],
...
function capabilityMenu(
	...
	managedResourceNames: ReadonlySet<string>,
): MenuItem[] {
	return items.flatMap((item) => {
		if (item.children) {
			const children = capabilityMenu(item.children, availableResources, managedResourceNames);
			return children.length > 0 ? [{ ...item, children }] : [];
		}
		if (managedResourceNames.has(item.name) && !availableResources.has(item.name)) return [];
		return [item];
	});
}
...
function orderedMenuItems(
	...
	const extensionItems = resources
		.filter((resource) => resource.menuGroup === menuGroup && !resource.hidden)
		.map((resource) => ({
			order: resource.menuOrder ?? Number.MAX_SAFE_INTEGER,
			menuItem: {
				name: resource.name,
	...
	const orderedCoreItems = coreItems.map((menuItem, index) => ({ order: (index + 1) * 100, menuItem }));
	return [...orderedCoreItems, ...extensionItems]
...
export function createMenu(
	...
	const menu: MenuItem[] = [
		...
		{
			name: 'models-group',
			label: labelFor(locale, 'elygate.modelManagement'),
			icon: 'box',
			children: orderedMenuItems(locale, 'models-group', enterpriseResources, [
		...
		},
		{
			...
			icon: 'boxes',
			children: orderedMenuItems(locale, 'mcp', enterpriseResources, [
		...
		},
		{
			...
			icon: 'shield-alert',
			children: orderedMenuItems(locale, 'guardrails', enterpriseResources, []),
		},
		{
			name: 'edge-control',
			label: labelFor(locale, 'elygate.edgeControl'),
			icon: 'hexagon',
			children: orderedMenuItems(locale, 'edge-control', enterpriseResources, []),
		},
		{
			name: 'integrations',
			label: labelFor(locale, 'elygate.integrations'),
			icon: 'plug-zap',
			children: orderedMenuItems(locale, 'integrations', enterpriseResources, [
		...
		},
		{
			...
			icon: 'settings',
			children: orderedMenuItems(locale, 'system', enterpriseResources, [
	...
	return capabilityMenu(
		menu,
		new Set(availableEnterpriseResources),
		new Set(enterpriseResources.map((resource) => resource.name)),
	);
}
