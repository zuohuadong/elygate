        ...
        <form class="space-y-6" onsubmit={handleSave}>
            ...
            >
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label
                            for="register-mode"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "注册模式" : "Registration Mode"}</label
                        >
                        <select
                            id="register-mode"
                            ...
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        >
                            <option value="open">{i18n.lang === "zh" ? "开放注册" : "Open Registration"}</option>
                            <option value="invite">{i18n.lang === "zh" ? "邀请码注册" : "Invite Only"}</option>
                            <option value="closed">{i18n.lang === "zh" ? "关闭注册" : "Closed"}</option>
                        </select>
                        <p class="text-xs text-slate-500">
                            {#if settings.RegisterMode === 'open'}
                                {i18n.lang === "zh" ? "任何人都可以注册，邀请码可选（可获额外额度）" : "Anyone can register. Invite code optional for bonus quota."}
                            {:else if settings.RegisterMode === 'invite'}
                                {i18n.lang === "zh" ? "必须使用有效邀请码才能注册" : "Valid invite code required for registration."}
                            {:else}
                                {i18n.lang === "zh" ? "禁止所有新用户注册" : "New user registration is disabled."}
                            {/if}
                        </p>
                    </div>
                    <div class="space-y-2">
                        <label
                            for="sign-quota"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "初始额度 ($)" : "Initial Quota ($)"}</label
                        >
                        <input
                            id="sign-quota"
                            type="number"
                            ...
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        />
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "新用户注册获得的初始额度（1美元 = 1000配额）" : "Initial quota for new users ($1 = 1000 quota)"}</p>
                    </div>
                </div>
                {#if settings.RegisterMode === 'invite'}
                    <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                        ...
                        >
                            {i18n.lang === "zh" ? "管理邀请码 →" : "Manage Invite Codes →"}
                        </a>
                    </div>
                {/if}
            </div>
            ...
            >
                <h3
                    class="text-base font-semibold text-slate-900 dark:text-white mb-4"
                >
                    {i18n.lang === "zh" ? "货币与汇率设置" : "Currency & Exchange Rate"}
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-2">
                        <label
                            ...
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "新用户默认货币" : "Default Currency for New Users"}</label
                        >
                        <select
                            id="default-currency"
                        ...
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "新注册用户的默认显示货币" : "Default display currency for new users"}</p>
                    </div>
                    <div class="space-y-2">
                        <label
                            for="exchange-rate"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "汇率 (1 USD = ? RMB)" : "Exchange Rate (1 USD = ? RMB)"}</label
                        >
                        <input
                            id="exchange-rate"
                            type="number"
                            ...
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        />
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "用于货币换算，仅影响显示，不影响实际额度" : "For display conversion only, does not affect actual quota"}</p>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-slate-700 dark:text-slate-300">
                            {i18n.lang === "zh" ? "换算参考" : "Conversion Reference"}
                        </label>
                        <div class="px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm">
                            <p class="text-slate-600 dark:text-slate-400">$1 = ¥{parseFloat(settings.ExchangeRate || '7.2').toFixed(2)}</p>
                            <p class="text-slate-500 dark:text-slate-500 text-xs mt-1">1,000 quota = ${(1000/500000).toFixed(4)} = ¥{(1000/500000 * parseFloat(settings.ExchangeRate || '7.2')).toFixed(4)}</p>
                        </div>
                    </div>
                </div>
                <div class="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-lg">
                    <p class="text-xs text-amber-700 dark:text-amber-400">
                        <strong>{i18n.lang === "zh" ? "注意：" : "Note:"}</strong>
                        {i18n.lang === "zh" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label
                            for="payment-enabled"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "开启充值功能" : "Enable Payment"}</label
                        >
                        <select
                            id="payment-enabled"
                        ...
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "关闭后用户将无法使用充值码充值" : "Disable to prevent users from redeeming codes"}</p>
                    </div>
                    <div class="space-y-2">
                        <label
                            for="payment-methods"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "充值方式" : "Payment Methods"}</label
                        >
                        <select
                            id="payment-methods"
                            ...
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        >
                            <option value="redemption">{i18n.lang === "zh" ? "兑换码充值" : "Redemption Code"}</option>
                            <option value="online">{i18n.lang === "zh" ? "在线支付" : "Online Payment"}</option>
                            <option value="both">{i18n.lang === "zh" ? "兑换码 + 在线支付" : "Both"}</option>
                        </select>
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "选择用户可用的充值方式" : "Select available payment methods"}</p>
                    </div>
                </div>
                {#if settings.PaymentEnabled === 'true'}
                    <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                        <a
                            ...
                            class="inline-flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400 hover:underline"
                        >
                            {i18n.lang === "zh" ? "管理兑换码 →" : "Manage Redemption Codes →"}
                        </a>
                    </div>
                {/if}
            </div>
            ...
            >
                ...
                <div class="space-y-6">
                    <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-xl">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-medium text-slate-900 dark:text-white">{i18n.lang === "zh" ? "密码登录" : "Password Login"}</p>
                                <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "使用用户名和密码登录" : "Login with username and password"}</p>
                            </div>
                        </div>
                        <select
                    ...
                    <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-xl">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1">
                                <p class="font-medium text-slate-900 dark:text-white">{i18n.lang === "zh" ? "GitHub 登录" : "GitHub Login"}</p>
                                <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "使用 GitHub OAuth 登录" : "Login with GitHub OAuth"}</p>
                            </div>
                        </div>
                        <select
                    ...
                    {#if settings.GitHubOAuthEnabled === 'true'}
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pl-14">
                            <div class="space-y-1">
                                <label for="github-client-id" class="text-xs font-medium text-slate-600 dark:text-slate-400">GitHub Client ID</label>
                                <input
                                    id="github-client-id"
                            ...
                            <div class="space-y-1">
                                <label for="github-client-secret" class="text-xs font-medium text-slate-600 dark:text-slate-400">GitHub Client Secret</label>
                                <input
                    ...
                    <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-xl">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1">
                                <p class="font-medium text-slate-900 dark:text-white">{i18n.lang === "zh" ? "微信登录" : "WeChat Login"}</p>
                                <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "使用微信 OAuth 登录" : "Login with WeChat OAuth"}</p>
                            </div>
                        </div>
                        <select
                    ...
                    {#if settings.WeChatOAuthEnabled === 'true'}
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pl-14">
                            <div class="space-y-1">
                                <label for="wechat-app-id" class="text-xs font-medium text-slate-600 dark:text-slate-400">{i18n.lang === "zh" ? "微信 AppID" : "WeChat App ID"}</label>
                                <input
                                    id="wechat-app-id"
                            ...
                            <div class="space-y-1">
                                <label for="wechat-app-secret" class="text-xs font-medium text-slate-600 dark:text-slate-400">{i18n.lang === "zh" ? "微信 AppSecret" : "WeChat App Secret"}</label>
                                <input
                ...
                <div class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-lg">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-2">
                        <label
                            for="alert-warning"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "警告阈值 (%)" : "Warning Threshold (%)"}</label
                        >
                        <input
                            id="alert-warning"
                            type="number"
                            ...
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        />
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "预算使用达到此比例时发送警告" : "Send warning when budget reaches this %"}</p>
                    </div>
                    <div class="space-y-2">
                        <label
                            for="alert-critical"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "严重阈值 (%)" : "Critical Threshold (%)"}</label
                        >
                        <input
                            id="alert-critical"
                            type="number"
                            ...
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        />
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "预算使用达到此比例时发送严重警告" : "Send critical alert at this %"}</p>
                    </div>
                    <div class="space-y-2">
                        <label
                            for="alert-exhausted"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "耗尽阈值 (%)" : "Exhausted Threshold (%)"}</label
                        >
                        <input
                            id="alert-exhausted"
                            type="number"
                            ...
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        />
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "预算使用达到此比例时发送耗尽警告" : "Send exhausted alert at this %"}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label
                            for="health-check"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "启用健康检查" : "Enable Health Check"}</label
                        >
                        <select
                            id="health-check"
                        ...
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "定期检查渠道可用性" : "Periodically check channel availability"}</p>
                    </div>
                    <div class="space-y-2">
                        <label
                            for="log-retention"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "日志保留天数" : "Log Retention Days"}</label
                        >
                        <input
                            id="log-retention"
                            type="number"
                            ...
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        />
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "超过此天数的日志将被自动删除" : "Logs older than this will be deleted"}</p>
                    </div>
                    <div class="space-y-2">
                        <label
                            for="circuit-breaker"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "熔断阈值" : "Circuit Breaker Threshold"}</label
                        >
                        <input
                            id="circuit-breaker"
                            type="number"
                            ...
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        />
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "连续失败次数达到此值时禁用渠道" : "Disable channel after this many failures"}</p>
                    </div>
                    <div class="space-y-2">
                        <label
                            for="circuit-recovery"
                            class="text-sm font-medium text-slate-700 dark:text-slate-300"
                            >{i18n.lang === "zh" ? "熔断恢复阈值" : "Recovery Threshold"}</label
                        >
                        <input
                            id="circuit-recovery"
                            type="number"
                            min="1"
                            max="100"
                            bind:value={settings.CircuitBreakerRecoveryThreshold}
                            class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        />
                        <p class="text-xs text-slate-500">{i18n.lang === "zh" ? "连续成功次数达到此值时恢复渠道" : "Re-enable channel after this many successes"}</p>
                    </div>
                </div>
            </div>
