...
<div class="flex-1 space-y-6 max-w-4xl mx-auto p-4 md:p-0">
    <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
            {i18n.lang === "zh" ? "遵循 OpenAI 接口规范，一键接入海量大模型。" : "Follows OpenAI standards. Easily integrate with numerous foundation models."}
        </p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-6 space-y-4">
            <div>
                ...
                <div class="relative">
                    <code class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-sm font-mono text-slate-800 dark:text-slate-200">
            ...
            <div class="pt-4">
                <p class="text-sm text-slate-600 dark:text-slate-400">
                    ...
                    <br/>
                    {i18n.lang === "zh" ? "所有 API 请求必须在 Header 中包含您的 API Key。" : "All requests must include your API Key in the Authorization headers."}
                </p>
                <code class="block mt-2 px-4 py-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-sm font-mono text-indigo-600 dark:text-indigo-400">
    ...
    <div class="bg-slate-900 rounded-2xl shadow-sm overflow-hidden border border-slate-800">
        <div class="p-4 border-b border-slate-800 flex items-center justify-between">
            ...
            <button 
                ...
                onclick={() => copyCode(`curl https://api.elygate.com/v1/chat/completions \\
...
<pre class="text-sm font-mono text-emerald-400"><code>curl https://api.elygate.com/v1/chat/completions \
  -H "Content-Type: application/json" \
