...
<div class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4">
    ...
    {#if resultQuota !== null && resultQuota > 0}
        <div class="mt-3 flex items-center justify-between">
            <div class="text-sm">
                <span class="text-slate-500 dark:text-slate-400">{i18n.lang === "zh" ? "系统额度" : "System Quota"}:</span>
                <span class="ml-2 font-mono font-semibold text-indigo-600 dark:text-indigo-400">{resultQuota.toLocaleString()}</span>
    ...
    <div class="mt-2 text-xs text-slate-400">
        $1 = {quotaPerUnit.toLocaleString()} quota | ¥1 = {(quotaPerUnit / exchangeRate).toFixed(0)} quota
    </div>
</div>
