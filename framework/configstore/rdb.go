...
// cloneWebhookEndpointForPersistence isolates GORM hooks from the caller's
// runtime object. Vault callbacks rewrite SecretVar values to vault references,
// while encryption hooks may replace Secret pointers and serialized headers.
func cloneWebhookEndpointForPersistence(endpoint *tables.TableWebhookEndpoint) *tables.TableWebhookEndpoint {
	if endpoint == nil {
		return nil
	}
	persist := *endpoint
	if endpoint.Secret != nil {
		...
		persist.Secret = &secret
	}
	if endpoint.Headers != nil {
		persist.Headers = make(map[string]schemas.SecretVar, len(endpoint.Headers))
		for name, value := range endpoint.Headers {
			persist.Headers[name] = value
		}
	}
	if endpoint.Events != nil {
		persist.Events = append([]tables.WebhookEvent(nil), endpoint.Events...)
	}
	return &persist
}
...
func (s *RDBConfigStore) CreateWebhookEndpoint(ctx context.Context, endpoint *tables.TableWebhookEndpoint) error {
	...
	// BeforeSave and the vault callback mutate Secret/Headers in place. Persist
	// an isolated copy so the caller keeps plaintext values for one-time display
	// and failed writes remain safe to retry.
	persist := cloneWebhookEndpointForPersistence(endpoint)
	return s.DB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func (s *RDBConfigStore) UpdateWebhookEndpoint(ctx context.Context, endpoint *tables.TableWebhookEndpoint) error {
	...
	return s.DB().Transaction(func(tx *gorm.DB) error {
		...
		persist := cloneWebhookEndpointForPersistence(&existing)
		return s.parseGormError(tx.WithContext(ctx).Save(persist).Error)
	})
}
...
func (s *RDBConfigStore) DeleteWebhookEndpoint(ctx context.Context, id string) error {
	return s.DB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var endpoint tables.TableWebhookEndpoint
		if err := dbForUpdate(tx).Where("id = ?", id).First(&endpoint).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				return ErrNotFound
			}
			return err
		}
		// Delete the hydrated entity so the global vault remove callback can
		// inspect its SecretVar fields and remove owned secrets.
...
func (s *RDBConfigStore) RotateWebhookEndpointSecret(ctx context.Context, id string) (*tables.TableWebhookEndpoint, error) {
	...
	err = s.DB().Transaction(func(tx *gorm.DB) error {
		...
		persist := cloneWebhookEndpointForPersistence(&rotated)
		return s.parseGormError(tx.WithContext(ctx).Save(persist).Error)
	})
	if err != nil {
		return nil, err
