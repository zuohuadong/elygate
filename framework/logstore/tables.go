...
type SearchFilters struct {
	...
	// Harness session tree. JSON/query names avoid colliding with session_id,
	// which the logs HTTP API already aliases to parent_request_id.
	HarnessSessionID string `json:"harness_session_id,omitempty"`
	ParentSessionID  string `json:"parent_session_id,omitempty"`
	AgentName        string `json:"agent_name,omitempty"`
	TreeSessionID    string `json:"tree_session_id,omitempty"`
...
// hasSessionTreeFilter reports whether the query must use the raw logs table.
// Harness session/parent/agent identity is not in mv_logs_hourly.
func (f SearchFilters) hasSessionTreeFilter() bool {
	return f.HarnessSessionID != "" || f.ParentSessionID != "" || f.AgentName != "" || f.TreeSessionID != "" || f.IsSubagent != nil || f.IsFork != nil
}
...
type Log struct {
	...
	ParentSessionID   *string  `gorm:"type:varchar(255);index:idx_logs_parent_session_id" json:"parent_session_id,omitempty"`
	AgentName         *string  `gorm:"type:varchar(255);index:idx_logs_agent_name" json:"agent_name,omitempty"`
	SessionClientType *string  `gorm:"type:varchar(32)" json:"session_client_type,omitempty"`
	IsSubagent        bool     `gorm:"not null;default:false;index:idx_logs_is_subagent" json:"is_subagent,omitempty"`
	IsFork            bool     `gorm:"not null;default:false;index:idx_logs_is_fork" json:"is_fork,omitempty"`
