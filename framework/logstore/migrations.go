...
var logstoreMigrationSteps = []migrationStep{
	...
	{IDs: []string{"logs_add_session_tree_columns"}, run: migrationAddSessionTreeColumns},
}
...
// migrationAddSessionTreeColumns stores harness session/parent/agent identity on
// the logs table so usage projection can read it without loading payload metadata.
func migrationAddSessionTreeColumns(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "logs_add_session_tree_columns"
	logger.Info("[logstore] starting migration %s", migrationName)
	defer logger.Info("[logstore] finished migration %s", migrationName)
	opts := *migrator.DefaultOptions
	opts.UseTransaction = true
	columns := []string{"parent_session_id", "agent_name", "session_client_type", "is_subagent", "is_fork"}
	m := migrator.New(db, &opts, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			for _, col := range columns {
				if err := addColumnIfNotExists(tx.WithContext(ctx), logger, &Log{}, col); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			for i := len(columns) - 1; i >= 0; i-- {
				if err := dropColumnIfExists(tx.WithContext(ctx), logger, &Log{}, columns[i]); err != nil {
					return err
				}
			}
			return nil
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while adding session tree columns: %s", err.Error())
	}
	return nil
}
