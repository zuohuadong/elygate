...
func (s *RDBLogStore) applyFilters(baseQuery *gorm.DB, filters SearchFilters) *gorm.DB {
	...
	if filters.HarnessSessionID != "" {
		baseQuery = baseQuery.Where("session_id = ?", filters.HarnessSessionID)
	}
	if filters.ParentSessionID != "" {
		baseQuery = baseQuery.Where("parent_session_id = ?", filters.ParentSessionID)
	}
	if filters.AgentName != "" {
		baseQuery = baseQuery.Where("agent_name = ?", filters.AgentName)
	}
	if filters.TreeSessionID != "" {
		baseQuery = baseQuery.Where("(session_id = ? OR parent_session_id = ?)", filters.TreeSessionID, filters.TreeSessionID)
	}
	if filters.IsSubagent != nil {
		baseQuery = baseQuery.Where("is_subagent = ?", *filters.IsSubagent)
	}
	if filters.IsFork != nil {
		baseQuery = baseQuery.Where("is_fork = ?", *filters.IsFork)
	}
...
func (s *RDBLogStore) listSelectColumns() string {
	baseCols := strings.Join([]string{
		...
		"user_agent", "app",
		"session_id", "parent_session_id", "agent_name", "session_client_type", "is_subagent", "is_fork",
