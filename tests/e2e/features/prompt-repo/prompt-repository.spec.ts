...
async function createPromptFromOpenSheet(page: Page, name: string): Promise<void> {
	...
	await expect(page.getByText("Prompt created", { exact: true })).toBeVisible();
}
...
test.describe.serial("Prompt Repository core interactions", () => {
	test("creates, updates, versions, renames sessions, and deletes prompt repository resources", async ({ page, request }) => {
		...
		try {
			...
			expect(session.messages.some((entry) => entry.message.content === userMessage)).toBe(true);
			...
			expect(version.messages.some((entry) => entry.message.content === userMessage)).toBe(true);
			...
			await expect(page.getByText("Prompt deleted", { exact: true })).toBeVisible();
