...
# Bifrost V1 Virtual Key Rotation Cooldown Newman Test Runner
# Runs VK rotation-cooldown tests: config API validation, rotation semantics, and
# grace-window enforcement at 1m and 3m windows. Wall clock is ~4 minutes.
...
# Configuration
COLLECTION="collections/bifrost-v1-vk-rotation-cooldown.postman_collection.json"
REPORT_DIR="newman-reports/vk-rotation-cooldown"
...
# Print banner
echo -e "${GREEN}==============================================${NC}"
echo -e "${GREEN}Bifrost V1 VK Rotation Cooldown Test Runner${NC}"
echo -e "${GREEN}==============================================${NC}"
echo ""
...
cmd+=(--timeout-script 300000 --timeout 900000)
cmd+=(-r "$REPORTERS")
...
if [ $EXIT_CODE -eq 0 ]; then
    echo -e "${GREEN}✓ All VK rotation-cooldown tests passed!${NC}"
else
