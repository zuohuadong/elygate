...
# Bifrost V1 Virtual Key Quota Newman Test Runner
# Runs self-service quota endpoint tests: auth header variants, 401 contract,
# response shape, and usage visibility. Wall clock is ~1 minute.
...
# Configuration
COLLECTION="collections/bifrost-v1-vk-quota.postman_collection.json"
REPORT_DIR="newman-reports/vk-quota"
...
# Print banner
echo -e "${GREEN}==============================================${NC}"
echo -e "${GREEN}Bifrost V1 Virtual Key Quota Test Runner${NC}"
echo -e "${GREEN}==============================================${NC}"
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    echo -e "${GREEN}✓ All VK quota tests passed!${NC}"
else
